...
const FLUX_SYSTEM = `You are FLUX AI — a powerful, intelligent assistant built into FLUX DIGITAL (fluxdigital.in). You were created by the FLUX DIGITAL team to help people worldwide.
...
function DiagnoseTool() {
  ...
  const PROMPT_TEMPLATE = (c,p,f) => `You are FLUX AI, an expert problem diagnosis engineer. Never mention Claude or Anthropic.
Category: ${c}. Problem: "${p||"Analyse the attached image/file"}"${f?.type==="text"?`\n\nAttached file (${f.name}):\n${f.text}`:""}
...
"safetyNote":"Safety warning","professionalNeeded":true,"whenToCallPro":"When to call pro"}`;
  ...
  const sc=s=>s==="Critical"?"var(--red)":s==="High"?"var(--amber)":s==="Medium"?"var(--blue)":"var(--green)";
  const sb=s=>s==="Critical"?"var(--red-t)":s==="High"?"var(--amber-t)":s==="Medium"?"var(--blue-t)":"var(--green-t)";
  const ow=(o="")=>{
    if(/maint|tech/i.test(o)) return {bg:"var(--amber-t)",c:"var(--amber)",br:"#fde68a"};
    if(/prod|oper/i.test(o)) return {bg:"var(--green-t)",c:"var(--green)",br:"#6ee7b7"};
    if(/qual/i.test(o)) return {bg:"var(--red-t)",c:"var(--red)",br:"#fecaca"};
    return {bg:"var(--blue-t)",c:"var(--blue)",br:"var(--blue-m)"};
  };
    ...
    <div className="diag-wrap">
      <div className="diag-input-card fu">
        <div className="diag-top">
          <div className="diag-icon" style={{background:"var(--blue-t)"}}>🔍</div>
        ...
        <div style={{padding:"18px 20px"}}>
          <div className="fg"><label className="fl">Category</label>
            <div style={{display:"flex",flexWrap:"wrap",gap:6,marginTop:4}}>
              {CATS.map(c=><button key={c.id} onClick={()=>setCat(c.id)} style={{padding:"6px 13px",borderRadius:99,border:"1.5px solid",borderColor:cat===c.id?c.c:"var(--line)",background:cat===c.id?`${c.c}14`:"var(--fog)",color:cat===c.id?c.c:"var(--ink2)",fontSize:".78rem",fontWeight:600,cursor:"pointer",transition:"all .15s"}}>{c.e} {c.label}</button>)}
            </div>
          </div>
          <div className="fg"><label className="fl">Quick Select</label>
            <div style={{display:"flex",flexWrap:"wrap",gap:5,marginTop:4}}>
              {(CHIPS[cat]||[]).map(p=><div key={p} onClick={()=>setProblem(p)} style={{fontSize:".76rem",padding:"4px 11px",borderRadius:99,border:"1.5px solid",borderColor:problem===p?"var(--blue)":"var(--line)",background:problem===p?"var(--blue-t)":"var(--fog)",color:problem===p?"var(--blue)":"var(--ink2)",cursor:"pointer",fontWeight:500}}>{p}</div>)}
            </div>
          </div>
          ...
          <div className="fg">
            ...
            {!uploadedFile?(
              <div className="upload-zone" onClick={()=>fileRef.current?.click()}>
                ...
                <div style={{fontWeight:700,fontSize:".86rem",color:"var(--ink)",marginBottom:3}}>Tap to upload photo or file</div>
                <div style={{fontSize:".73rem",color:"var(--ink3)",lineHeight:1.6}}>📸 Fault photo · 📊 Excel data · 📄 PDF · AI reads and analyses</div>
              </div>
            ):(
              <div className="upload-preview">
                {uploadedFile.preview?<img src={uploadedFile.preview} alt="" style={{width:60,height:60,objectFit:"cover",borderRadius:8,border:"1px solid var(--blue-m)",flexShrink:0}}/>
                  :<div style={{width:44,height:44,borderRadius:8,background:"var(--blue)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"1.3rem",flexShrink:0}}>📄</div>}
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontWeight:700,fontSize:".84rem",color:"var(--blue)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{uploadedFile.name}</div>
                  <div style={{fontSize:".7rem",color:"var(--ink2)",marginTop:2}}>{uploadedFile.type==="image"?"✅ FLUX AI will visually analyse this image":"✅ FLUX AI will read and analyse file content"}</div>
                </div>
                <button onClick={rmFile} style={{background:"var(--red-t)",border:"1px solid #fecaca",color:"var(--red)",borderRadius:6,padding:"4px 9px",cursor:"pointer",fontSize:".72rem",fontWeight:700}}>✕</button>
              </div>
            )}
            {uploadErr&&<div style={{color:"var(--red)",fontSize:".76rem",marginTop:4}}>⚠ {uploadErr}</div>}
          </div>
          {progress>0&&<div style={{height:4,background:"var(--fog2)",borderRadius:99,overflow:"hidden",marginBottom:10}}><div style={{height:"100%",background:"linear-gradient(90deg,var(--blue),var(--indigo))",width:`${progress}%`,transition:"width .5s",borderRadius:99}}/></div>}
      ...
      {report&&!report.error&&(
        <div className="diag-input-card pop">
          <div className="diag-top">
            ...
            <div style={{flex:1}}>
              <div style={{fontFamily:"var(--fm)",fontSize:".6rem",color:"var(--ink3)",marginBottom:2}}>Report: {rid.current} · {new Date().toLocaleDateString()}</div>
          ...
          <div className="sev-row">
            <span style={{fontSize:".72rem",fontWeight:700,color:"var(--ink3)",textTransform:"uppercase",letterSpacing:".07em"}}>Severity</span>
          ...
          <div style={{padding:"18px 20px",display:"flex",flexDirection:"column",gap:18}}>
            {/* Summary */}
            <div><div style={{fontSize:".65rem",fontWeight:800,color:"var(--ink3)",letterSpacing:".09em",textTransform:"uppercase",marginBottom:8}}>📋 Summary</div>
              <div style={{padding:"13px 15px",background:"var(--fog)",border:"1.5px solid var(--line)",borderRadius:9,fontSize:".9rem",color:"var(--ink)",lineHeight:1.7}}>{report.summary}</div>
            </div>
            {/* Causes */}
            <div><div style={{fontSize:".65rem",fontWeight:800,color:"var(--ink3)",letterSpacing:".09em",textTransform:"uppercase",marginBottom:8}}>🎯 Most Likely Causes</div>
              {report.topCauses?.map((c,i)=>(
                <div key={i} style={{display:"flex",gap:11,padding:"10px 13px",border:"1.5px solid var(--line)",borderRadius:9,background:"var(--fog)",marginBottom:6,alignItems:"flex-start",transition:"all .18s"}}>
                  <div style={{width:26,height:26,borderRadius:7,background:i===0?"var(--red-t)":i===1?"var(--amber-t)":"var(--fog2)",color:i===0?"var(--red)":i===1?"var(--amber)":"var(--ink3)",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:".72rem",flexShrink:0}}>{c.rank}</div>
                  <div style={{flex:1}}><div style={{fontSize:".88rem",color:"var(--ink)",fontWeight:600}}>{c.cause}</div><div style={{fontSize:".73rem",color:"var(--ink3)",marginTop:2}}>{c.why}</div></div>
                  <div style={{fontFamily:"var(--fm)",fontSize:".78rem",fontWeight:700,color:i===0?"var(--red)":i===1?"var(--amber)":"var(--ink3)",flexShrink:0}}>{c.probability}</div>
                </div>
              ))}
            </div>
            {/* Steps */}
            <div><div style={{fontSize:".65rem",fontWeight:800,color:"var(--ink3)",letterSpacing:".09em",textTransform:"uppercase",marginBottom:8}}>🔧 Step-by-Step Fix</div>
              {report.fixSteps?.map((s,i)=>{const last=i===report.fixSteps.length-1;return(
                <div key={i} style={{display:"flex",gap:13,marginBottom:4}}>
                  <div style={{display:"flex",flexDirection:"column",alignItems:"center",width:30,flexShrink:0}}>
                    <div style={{width:30,height:30,borderRadius:"50%",background:last?"var(--green-t)":"var(--blue-t)",color:last?"var(--green)":"var(--blue)",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:".78rem"}}>{s.num}</div>
                    {!last&&<div style={{width:2,flex:1,minHeight:10,background:"var(--fog2)"}}/>}
                  </div>
                  <div style={{flex:1,paddingBottom:12}}>
                    <div style={{fontWeight:700,fontSize:".9rem",color:"var(--ink)",marginBottom:2}}>{s.title}</div>
                    <div style={{fontSize:".84rem",color:"var(--ink2)",lineHeight:1.6}}>{s.detail}</div>
                    {s.tip&&<div style={{display:"inline-flex",alignItems:"center",gap:5,marginTop:5,fontSize:".73rem",fontWeight:600,color:"var(--amber)",background:"var(--amber-t)",padding:"3px 9px",borderRadius:99}}>💡 {s.tip}</div>}
                  </div>
                </div>
              );})}
            </div>
            {/* Why Chain */}
            <div><div style={{fontSize:".65rem",fontWeight:800,color:"var(--ink3)",letterSpacing:".09em",textTransform:"uppercase",marginBottom:8}}>🔍 Why-Why Analysis — Cause + Solution</div>
              {report.whyChain?.map((w,i)=>{const last=i===report.whyChain.length-1;const os=ow(w.owner);return(
                <div key={i} style={{display:"flex",gap:11}}>
                  <div style={{display:"flex",flexDirection:"column",alignItems:"center",width:34,flexShrink:0}}>
                    <div style={{width:33,height:33,borderRadius:"50%",background:last?"var(--red-t)":"var(--blue-t)",color:last?"var(--red)":"var(--blue)",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:".72rem"}}>W{w.step}</div>
                    {!last&&<div style={{width:2,flex:1,minHeight:10,background:"var(--fog2)"}}/>}
                  </div>
                  <div style={{flex:1,paddingBottom:12}}>
                    <div style={{fontSize:".72rem",fontWeight:700,textTransform:"uppercase",letterSpacing:".06em",color:"var(--ink3)",marginBottom:4}}>Why {w.step}</div>
                    <div style={{fontWeight:600,fontSize:".88rem",color:"var(--ink)",marginBottom:7}}>{w.question}</div>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:7,marginBottom:6}}>
                      <div className="cause-box" style={{borderLeftColor:"var(--amber)",background:"var(--amber-t)"}}><div style={{fontSize:".64rem",fontWeight:800,color:"var(--amber)",marginBottom:2}}>🔴 CAUSE</div>{w.cause}</div>
                      <div className="sol-box" style={{borderLeftColor:"var(--green)",background:"var(--green-t)"}}><div style={{fontSize:".64rem",fontWeight:800,color:"var(--green)",marginBottom:2}}>✅ SOLUTION</div>{w.solution}</div>
                    </div>
                    <div style={{display:"flex",gap:5}}>
                      ...
                      <span style={{fontSize:".65rem",padding:"2px 8px",borderRadius:99,background:"var(--fog)",color:"var(--ink3)",border:"1px solid var(--line)",fontFamily:"var(--fm)"}}>⏱ {w.timeframe}</span>
                    </div>
                  </div>
                </div>
              );})}
            ...
            {/* Fishbone */}
            <div><div style={{fontSize:".65rem",fontWeight:800,color:"var(--ink3)",letterSpacing:".09em",textTransform:"uppercase",marginBottom:8}}>🐟 Fishbone — 6M Causes</div>
              <div className="fish-grid">
                {report.fishbone&&Object.entries(report.fishbone).map(([b,cs])=>{
                  const fc={Man:"var(--amber)",Machine:"var(--blue)",Method:"var(--green)",Material:"#f59e0b",Environment:"var(--purple)",Measurement:"var(--red)"};
                  return(<div className="fish-cell" key={b}><div className="fish-head" style={{color:fc[b]||"var(--ink)"}}>{b}</div>{cs.map((c,i)=><div className="fish-pt" key={i}>{c}</div>)}</div>);
                })}
              </div>
            </div>
            {/* Actions */}
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
              <div>
                <div style={{fontSize:".65rem",fontWeight:800,color:"var(--red)",letterSpacing:".09em",textTransform:"uppercase",marginBottom:8}}>⚡ Corrective Actions</div>
                {report.correctiveActions?.map((a,i)=>{const os=ow(a.owner);return(
                  <div key={i} className="act-row">
                    <div className="act-text"><span style={{fontWeight:800,color:"var(--ink3)",fontSize:".75rem"}}>{i+1}. </span>{a.action}</div>
              ...
              <div>
                <div style={{fontSize:".65rem",fontWeight:800,color:"var(--green)",letterSpacing:".09em",textTransform:"uppercase",marginBottom:8}}>🛡 Prevention Tips</div>
                {report.preventiveTips?.map((t,i)=><div key={i} style={{padding:"8px 12px",marginBottom:6,background:"var(--green-t)",borderRadius:8,border:"1px solid #6ee7b7",fontSize:".84rem",color:"var(--ink)",display:"flex",gap:7}}><span style={{color:"var(--green)",fontWeight:800,flexShrink:0}}>✓</span>{t}</div>)}
              </div>
            </div>
            ...
            {report.professionalNeeded&&<div style={{padding:"11px 14px",background:"var(--red-t)",border:"1.5px solid #fecaca",borderRadius:9,fontSize:".86rem",color:"var(--ink)"}}><strong style={{color:"var(--red)"}}>📞 Call a professional when:</strong> {report.whenToCallPro}</div>}
          </div>
          <div className="rep-footer">
            <div className="rep-id">FLUX AI Report · {rid.current} · fluxdigital.in</div>
            <button className="btn btn-ghost" style={{fontSize:".76rem",padding:"6px 13px"}} onClick={()=>window.print()}>🖨 Print</button>
          </div>
        </div>
      )}
      {report?.error&&<div style={{padding:18,background:"var(--red-t)",border:"1.5px solid #fecaca",borderRadius:12,color:"var(--red)",fontSize:".88rem"}}>⚠️ Analysis failed. Please check connection and retry.</div>}
      </div>
    </div>
  );
}
...
function CalcPanel({id}) {
  ...
  return(
    <div>
      ...
      {hist.length>0&&<div style={{border:"1.5px solid var(--line)",borderRadius:9,overflow:"hidden",marginTop:12}}><div style={{padding:"7px 12px",background:"var(--fog)",borderBottom:"1px solid var(--line)",fontSize:".68rem",fontWeight:700,color:"var(--ink3)",letterSpacing:".07em",textTransform:"uppercase"}}>History</div>{hist.map((h,i)=><div key={i} style={{display:"flex",justifyContent:"space-between",padding:"6px 12px",borderBottom:"1px solid var(--line)",fontSize:".8rem",color:"var(--ink3)"}}><span>{h.ts}</span><span style={{fontFamily:"var(--fm)",fontWeight:600,color:"var(--blue)"}}>{h.val} {h.unit}</span></div>)}</div>}
    </div>
  );
}
  ...
  return(
    <div className="pm-shell">
      ...
      <div style={{padding:"14px 16px"}}>
        ...
        {aiItems&&<div style={{padding:"7px 11px",background:"var(--blue-t)",border:"1.5px solid var(--blue-m)",borderRadius:8,fontSize:".78rem",color:"var(--blue)",fontWeight:600,marginBottom:10}}>✨ AI checklist for: {machine}</div>}
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
          <span style={{fontSize:".8rem",fontWeight:700,color:"var(--ink)"}}>{done}/{disp.length} completed</span>
          <span style={{fontSize:".8rem",fontWeight:700,color:"var(--green)"}}>{pct}%</span>
        </div>
        ...
        {disp.map((item,idx)=>{const id=item.id||item.t||String(idx);return(
          <div key={id} className="pm-item" onClick={()=>tog(id)}>
            ...
            <div><div className={`pm-t ${chk[id]?"dn":""}`}>{item.t||item.task}{item.critical&&<span style={{fontSize:".58rem",fontWeight:800,color:"var(--red)",marginLeft:5,padding:"1px 5px",borderRadius:99,background:"var(--red-t)"}}>CRITICAL</span>}</div><div className="pm-m">{item.m||item.machine}</div></div>
          </div>
        );})}
...
function EmailWriter() {
  ...
  const cs={background:"#fff",border:"1.5px solid var(--line)",borderRadius:var(--r),padding:18};
  return(
    <div style={{display:"grid",gridTemplateColumns:"300px 1fr",gap:14}}>
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        <div style={{background:"#fff",border:"1.5px solid var(--line)",borderRadius:12,padding:18}}>
          <div style={{fontFamily:"var(--fh)",fontWeight:800,fontSize:"1rem",color:"var(--ink)",marginBottom:4}}>✉️ Email Writer</div>
          <div style={{fontSize:".78rem",color:"var(--ink3)",marginBottom:14}}>AI writes professional emails instantly</div>
          <div style={{display:"flex",gap:5,marginBottom:14}}>
            {[["write","✍️ Write New"],["correct","✏️ Correct Mine"]].map(([id,l])=><button key={id} onClick={()=>{setMode(id);setResult(null);}} style={{flex:1,padding:"7px 0",borderRadius:8,border:"1.5px solid",borderColor:mode===id?"var(--blue)":"var(--line)",background:mode===id?"var(--blue-t)":"var(--fog)",color:mode===id?"var(--blue)":"var(--ink2)",fontSize:".76rem",fontWeight:600,cursor:"pointer"}}>{l}</button>)}
          </div>
          {mode==="write"?(
            <>
              <div className="fg"><label className="fl">Email Type</label>
                <div style={{display:"flex",flexWrap:"wrap",gap:4,marginTop:4}}>
                  {EMAIL_TYPES.map(t=><button key={t.id} onClick={()=>setEType(t.id)} style={{padding:"4px 9px",borderRadius:7,border:"1.5px solid",borderColor:eType===t.id?"var(--blue)":"var(--line)",background:eType===t.id?"var(--blue-t)":"var(--fog)",color:eType===t.id?"var(--blue)":"var(--ink2)",fontSize:".72rem",fontWeight:600,cursor:"pointer"}}>{t.l}</button>)}
                </div>
              </div>
              <div className="fg"><label className="fl">Tone</label>
                <div style={{display:"flex",flexWrap:"wrap",gap:4,marginTop:4}}>
                  {TONES.map(t=><button key={t} onClick={()=>setTone(t)} style={{padding:"3px 9px",borderRadius:99,border:"1.5px solid",borderColor:tone===t?"var(--blue)":"var(--line)",background:tone===t?"var(--blue-t)":"var(--fog)",color:tone===t?"var(--blue)":"var(--ink2)",fontSize:".72rem",fontWeight:600,cursor:"pointer"}}>{t}</button>)}
                </div>
              </div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:11}}>
        {!result&&!loading&&<div style={{background:"#fff",border:"1.5px solid var(--line)",borderRadius:12,padding:40,textAlign:"center"}}><div style={{fontSize:"2.2rem",marginBottom:11}}>✉️</div><div style={{fontWeight:700,color:"var(--ink2)",marginBottom:5}}>{mode==="write"?"Fill in details to write your email":"Paste your email to get it corrected"}</div><div style={{fontSize:".8rem",color:"var(--ink3)",lineHeight:1.7}}>FLUX AI writes {mode==="write"?"2 versions: Short & Detailed":"corrected version with tracked changes"}</div></div>}
        {loading&&<div style={{background:"#fff",border:"1.5px solid var(--line)",borderRadius:12,padding:40,textAlign:"center"}}><div style={{fontSize:"1.8rem",marginBottom:10}}>✍️</div><div style={{fontWeight:700,color:"var(--blue)"}}>FLUX AI is writing...</div></div>}
        {result&&!result.error&&(
          <>
            {result.subject&&<div style={{background:"#fff",border:"1.5px solid var(--line)",borderRadius:10,padding:13,display:"flex",alignItems:"center",justifyContent:"space-between",gap:10}}>
              <div><div style={{fontSize:".64rem",fontWeight:800,color:"var(--ink3)",letterSpacing:".07em",textTransform:"uppercase",marginBottom:2}}>Subject Line</div><div style={{fontWeight:700,fontSize:".92rem",color:"var(--ink)"}}>{result.subject}</div></div>
            ...
            {result.corrected&&<div style={{background:"#fff",border:"1.5px solid var(--green)",borderRadius:10,padding:15}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:9}}><div style={{fontSize:".64rem",fontWeight:800,color:"var(--green)",letterSpacing:".07em",textTransform:"uppercase"}}>✅ Corrected Email</div><button className={`copy-btn ${copied==="corr"?"copied":""}`} onClick={()=>copy(result.corrected,"corr")}>{copied==="corr"?"✓ Copied":"📋 Copy"}</button></div>
              <pre style={{fontFamily:"var(--fb)",fontSize:".86rem",color:"var(--ink)",lineHeight:1.7,whiteSpace:"pre-wrap"}}>{result.corrected}</pre>
              {result.changes&&<div style={{marginTop:11,padding:"10px 13px",background:"var(--blue-t)",border:"1.5px solid var(--blue-m)",borderRadius:8}}><div style={{fontSize:".64rem",fontWeight:800,color:"var(--blue)",marginBottom:6,letterSpacing:".07em",textTransform:"uppercase"}}>Changes Made</div>{result.changes.map((c,i)=><div key={i} style={{fontSize:".82rem",color:"var(--ink)",padding:"3px 0",display:"flex",gap:7}}><span style={{color:"var(--blue)",fontWeight:800,flexShrink:0}}>•</span>{c}</div>)}</div>}
            </div>}
            {result.short&&<div style={{background:"#fff",border:"1.5px solid var(--line)",borderRadius:10,padding:15}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:9}}><div style={{fontSize:".64rem",fontWeight:800,color:"var(--blue)",letterSpacing:".07em",textTransform:"uppercase"}}>Version 1 — Short & Direct</div><button className={`copy-btn ${copied==="s"?"copied":""}`} onClick={()=>copy(result.short.body,"s")}>{copied==="s"?"✓ Copied":"📋 Copy"}</button></div>
              <pre style={{fontFamily:"var(--fb)",fontSize:".86rem",color:"var(--ink)",lineHeight:1.7,whiteSpace:"pre-wrap"}}>{result.short.body}</pre>
            </div>}
            {result.detailed&&<div style={{background:"#fff",border:"1.5px solid var(--line)",borderRadius:10,padding:15}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:9}}><div style={{fontSize:".64rem",fontWeight:800,color:"var(--indigo)",letterSpacing:".07em",textTransform:"uppercase"}}>Version 2 — Detailed & Formal</div><button className={`copy-btn ${copied==="l"?"copied":""}`} onClick={()=>copy(result.detailed.body,"l")}>{copied==="l"?"✓ Copied":"📋 Copy"}</button></div>
              <pre style={{fontFamily:"var(--fb)",fontSize:".86rem",color:"var(--ink)",lineHeight:1.7,whiteSpace:"pre-wrap"}}>{result.detailed.body}</pre>
            </div>}
            {result.tips&&<div style={{background:"var(--amber-t)",border:"1.5px solid #fde68a",borderRadius:10,padding:13}}><div style={{fontSize:".64rem",fontWeight:800,color:"var(--amber)",letterSpacing:".07em",textTransform:"uppercase",marginBottom:7}}>💡 Tips</div>{result.tips.map((t,i)=><div key={i} style={{fontSize:".83rem",color:"var(--ink)",padding:"3px 0",display:"flex",gap:7}}><span style={{color:"var(--amber)",fontWeight:800}}>•</span>{t}</div>)}</div>}
          </>
        )}
      </div>
    </div>
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:13}}>
      <div style={{background:"#fff",border:"1.5px solid var(--line)",borderRadius:12,padding:20}}>
        <div style={{fontFamily:"var(--fh)",fontWeight:800,fontSize:"1rem",color:"var(--ink)",marginBottom:3}}>🏭 TPM & Lean Manufacturing Knowledge</div>
        <div style={{fontSize:".78rem",color:"var(--ink3)",marginBottom:13}}>Search any topic — get full implementation guide, KPIs, real examples</div>
      ...
      {loading&&<div style={{background:"#fff",border:"1.5px solid var(--line)",borderRadius:12,padding:36,textAlign:"center",color:"var(--ink3)"}}>🔍 Loading knowledge...</div>}
      {result&&!result.error&&(
        <div style={{display:"flex",flexDirection:"column",gap:11}} className="fi">
          <div style={{background:"linear-gradient(135deg,var(--blue-t),var(--indigo-t))",border:"1.5px solid var(--blue-m)",borderRadius:12,padding:20}}>
            <div style={{fontFamily:"var(--fh)",fontWeight:800,fontSize:"1.3rem",color:"var(--ink)",marginBottom:7}}>{result.title}</div>
            <div style={{fontSize:".9rem",color:"var(--ink2)",lineHeight:1.75,fontStyle:"italic"}}>{result.definition}</div>
          </div>
          ...
          {secTab==="overview"&&<><div style={{background:"#fff",border:"1.5px solid var(--line)",borderRadius:11,padding:18}}><div style={{fontSize:".65rem",fontWeight:800,color:"var(--ink3)",letterSpacing:".09em",textTransform:"uppercase",marginBottom:8}}>Overview</div><div style={{fontSize:".9rem",color:"var(--ink)",lineHeight:1.8}}>{result.overview}</div></div>
          <div style={{background:"#fff",border:"1.5px solid var(--line)",borderRadius:11,padding:18}}><div style={{fontSize:".65rem",fontWeight:800,color:"var(--blue)",letterSpacing:".09em",textTransform:"uppercase",marginBottom:9}}>Key Points</div>{result.keyPoints?.map((p,i)=><div key={i} style={{display:"flex",gap:10,padding:"7px 0",borderBottom:"1px solid var(--line)",fontSize:".87rem",color:"var(--ink)"}}><span style={{fontFamily:"var(--fm)",fontSize:".68rem",fontWeight:700,color:"var(--blue)",background:"var(--blue-t)",padding:"2px 8px",borderRadius:6,flexShrink:0,alignSelf:"flex-start",marginTop:1}}>{i+1}</span>{p}</div>)}</div>
          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{result.relatedTopics?.map(t=><button key={t} onClick={()=>search(t)} style={{padding:"5px 12px",borderRadius:99,border:"1.5px solid var(--blue-m)",background:"var(--blue-t)",color:"var(--blue)",fontSize:".75rem",fontWeight:600,cursor:"pointer"}}>→ {t}</button>)}</div></>}
          {secTab==="steps"&&<div style={{background:"#fff",border:"1.5px solid var(--line)",borderRadius:11,padding:18}}><div style={{fontSize:".65rem",fontWeight:800,color:"var(--green)",letterSpacing:".09em",textTransform:"uppercase",marginBottom:12}}>Implementation Steps</div>{result.howToImplement?.map((s,i)=><div key={i} style={{display:"flex",gap:12,marginBottom:14}}><div style={{width:32,height:32,borderRadius:"50%",background:"var(--blue)",color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:".78rem",flexShrink:0}}>{s.step}</div><div><div style={{fontWeight:700,fontSize:".9rem",color:"var(--ink)",marginBottom:2}}>{s.title}</div><div style={{fontSize:".84rem",color:"var(--ink2)",lineHeight:1.65}}>{s.detail}</div></div></div>)}</div>}
          {secTab==="benefits"&&<div style={{background:"#fff",border:"1.5px solid var(--line)",borderRadius:11,padding:18}}><div style={{fontSize:".65rem",fontWeight:800,color:"var(--green)",letterSpacing:".09em",textTransform:"uppercase",marginBottom:9}}>Benefits</div>{result.benefits?.map((b,i)=><div key={i} style={{display:"flex",gap:8,padding:"8px 0",borderBottom:"1px solid var(--line)",fontSize:".87rem",color:"var(--ink)"}}><span style={{color:"var(--green)",fontWeight:800,flexShrink:0}}>✓</span>{b}</div>)}</div>}
          {secTab==="kpis"&&<div style={{background:"#fff",border:"1.5px solid var(--line)",borderRadius:11,padding:18}}><div style={{fontSize:".65rem",fontWeight:800,color:"var(--purple)",letterSpacing:".09em",textTransform:"uppercase",marginBottom:9}}>KPIs & Targets</div>{result.kpis?.map((k,i)=><div key={i} style={{display:"flex",gap:9,padding:"8px 11px",marginBottom:6,background:"var(--purple-t)",borderRadius:8,fontSize:".87rem",color:"var(--ink)"}}><span style={{color:"var(--purple)",fontWeight:800,flexShrink:0}}>📊</span>{k}</div>)}</div>}
          {secTab==="mistakes"&&<div style={{background:"#fff",border:"1.5px solid var(--line)",borderRadius:11,padding:18}}><div style={{fontSize:".65rem",fontWeight:800,color:"var(--red)",letterSpacing:".09em",textTransform:"uppercase",marginBottom:9}}>Common Mistakes</div>{result.commonMistakes?.map((m,i)=><div key={i} style={{display:"flex",gap:8,padding:"8px 0",borderBottom:"1px solid var(--line)",fontSize:".87rem",color:"var(--ink)"}}><span style={{color:"var(--red)",fontWeight:800,flexShrink:0}}>✕</span>{m}</div>)}</div>}
          {secTab==="example"&&<div style={{background:"var(--green-t)",border:"1.5px solid #6ee7b7",borderRadius:11,padding:18}}><div style={{fontSize:".65rem",fontWeight:800,color:"var(--green)",letterSpacing:".09em",textTransform:"uppercase",marginBottom:8}}>Real-World Example</div><div style={{fontSize:".9rem",color:"var(--ink)",lineHeight:1.8}}>{result.realExample}</div></div>}
        </div>
      )}
      {result?.error&&<div style={{padding:18,background:"var(--red-t)",border:"1.5px solid #fecaca",borderRadius:11,color:"var(--red)"}}>Search failed. Please retry.</div>}
    </div>
  );
}
    ...
    <div style={{minHeight:"100vh",background:"var(--fog)"}}>
        ...
        <>
          ...
          <div className="pw">
            ...
            <div style={{background:"#fff",border:"1.5px solid var(--line)",borderRadius:16,padding:26,marginBottom:40}}>
              ...
              <h2 className="sec-t">How fluxdigital.in Earns</h2>
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(190px,1fr))",gap:11,marginTop:16}}>
                {[["📢","Google AdSense","Ads on every page","₹2–20/1000 views"],["💎","Premium Plans","Engineer & Enterprise","₹499–1,999/month"],["🛒","Affiliates","Equipment & tools","5–15% commission"],["📧","Lead Generation","Connect to vendors","₹50–500/lead"],["🎓","Online Courses","TPM, Lean, PLC","₹999–4,999/course"],["🤝","Sponsored Content","Industry partners","₹5,000–50,000/post"]].map(([e,t,d,earn])=>(
                  <div key={t} style={{background:"var(--fog)",border:"1.5px solid var(--line)",borderRadius:10,padding:15}}>
                    <div style={{fontSize:"1.4rem",marginBottom:8}}>{e}</div>
                    <div style={{fontWeight:800,fontSize:".88rem",color:"var(--ink)",marginBottom:3}}>{t}</div>
                    <div style={{fontSize:".76rem",color:"var(--ink2)",lineHeight:1.55,marginBottom:7}}>{d}</div>
                    <div style={{fontSize:".7rem",fontWeight:700,color:"var(--green)",background:"var(--green-t)",padding:"2px 8px",borderRadius:99,display:"inline-flex"}}>{earn}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      {page==="AI"&&(
        <div className="pw" style={{maxWidth:900}}>
          ...
          <div style={{display:"flex",gap:7,flexWrap:"wrap",marginBottom:14}}>{CATS.map(c=><button key={c.id} onClick={()=>setCat(c.id)} style={{padding:"6px 13px",borderRadius:99,border:"1.5px solid",borderColor:cat===c.id?c.c:"var(--line)",background:cat===c.id?`${c.c}14`:"var(--fog)",color:cat===c.id?c.c:"var(--ink2)",fontSize:".78rem",fontWeight:600,cursor:"pointer"}}>{c.e} {c.label}</button>)}</div>
          <FluxChat cat={cat}/>
      ...
      {page==="DIAGNOSE"&&(
        <div className="pw" style={{maxWidth:960}}>
          ...
          <DiagnoseTool/>
          <div style={{marginTop:18}}><div style={{fontWeight:700,fontSize:".86rem",color:"var(--ink2)",marginBottom:11}}>💬 Or chat directly with FLUX AI:</div><FluxChat cat={cat}/></div>
        </div>
      )}
        ...
        <div className="pw">
          ...
          <div style={{display:"grid",gridTemplateColumns:"240px 1fr",gap:14}}>
            <div style={{display:"flex",flexDirection:"column",gap:5}}>
              {CALC_LIST.map(c=><div key={c.id} onClick={()=>setCalcId(c.id)} style={{display:"flex",alignItems:"center",gap:9,padding:"9px 11px",borderRadius:9,border:"1.5px solid",borderColor:calcId===c.id?"var(--blue)":"var(--line)",background:calcId===c.id?"var(--blue-t)":"#fff",cursor:"pointer",transition:"all .15s",boxShadow:"var(--sh)"}}>
                <span style={{fontSize:"1.15rem"}}>{c.e}</span>
                <div><div style={{fontWeight:700,fontSize:".82rem",color:calcId===c.id?"var(--blue)":"var(--ink)"}}>{c.n}</div><div style={{fontSize:".68rem",color:"var(--ink3)"}}>{c.d}</div></div>
              </div>)}
            </div>
            <div style={{background:"#fff",border:"1.5px solid var(--line)",borderRadius:var(--r),padding:22,boxShadow:"var(--sh)"}}>
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
                <span style={{fontSize:"1.4rem"}}>{CALC_LIST.find(c=>c.id===calcId)?.e}</span>
                <div><div style={{fontFamily:"var(--fh)",fontWeight:800,fontSize:"1.05rem",color:"var(--ink)"}}>{CALC_LIST.find(c=>c.id===calcId)?.n}</div><div style={{fontSize:".78rem",color:"var(--ink3)"}}>{CALC_LIST.find(c=>c.id===calcId)?.d}</div></div>
              </div>
              <CalcPanel id={calcId}/>
            </div>
          </div>
              ...
              <div key={p.tier} className={`price-card ${p.pop?"pop":""}`}>
                ...
                {p.inr===0
                  ...
                  :<div style={{marginTop:18}}>
                    ...
                    <div style={{fontSize:".68rem",color:"var(--ink3)",textAlign:"center",marginTop:5}}>🔒 Secure via Razorpay → Direct to your bank/GPay</div>
                  </div>
                }
              </div>
            ))}
      ...
      <footer style={{background:"var(--ink)",padding:"36px 20px 22px",marginTop:56}}>
        <div style={{maxWidth:1100,margin:"0 auto"}}>
          <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr",gap:28,marginBottom:28}}>
            <div>
              <div style={{fontFamily:"var(--fh)",fontSize:"1.1rem",fontWeight:800,color:"#fff",marginBottom:8}}>✦ FLUX DIGITAL</div>
            ...
            {[["FLUX AI Skills",["Problem Diagnosis","Email Writer","TPM Knowledge","PM Checklists","12 Calculators","Work Automation"]],["Problem Types",["Electrical Faults","Plumbing Issues","WiFi & Internet","Industrial PLC","EB Meter Help","Home Repair"]],["Company",["About fluxdigital.in","Pricing Plans","Advertise With Us","Privacy Policy","Contact Us","Terms of Use"]]].map(([h,ls])=>(
              <div key={h}>
          ...
          <div style={{borderTop:"1px solid #1e293b",paddingTop:18,display:"flex",justifyContent:"space-between",flexWrap:"wrap",gap:7,fontSize:".76rem",color:"#64748b"}}>
            <span>© 2025 Flux Digital — fluxdigital.in · All rights reserved</span>
